/** Static HEX map of Tailwind CSS v4 color palette (22 families × 11 shades = 242 entries) */
...
/** Reverse lookup: hex to Tailwind color name (exact match) */
const reverseLookup = new Map<string, string>();
for (const [name, hex] of Object.entries(TAILWIND_COLORS)) {
  // First entry wins (some hex values may be shared)
  if (!reverseLookup.has(hex)) {
    reverseLookup.set(hex, name);
  }
}
