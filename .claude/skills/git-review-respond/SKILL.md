...
2. **プロジェクト整合性**: 指摘内容がプロジェクトの既存パターンやルール（CLAUDE.md, conventions.md等）と整合しているか？プロジェクトの規約に反する提案は採用しない
...
**署名**: `.claude/rules/conventions.md` の「GitHubコメントの署名」ルールに従い、各返信の末尾に署名を付与する。
